...
%import data
%--------------------------------------------------------------------------
load('C:\Users\Jimmy\Box Sync\share\phm competition\Saurabh\DataComb\NaN_Removal\NTrain\NCombPlant36.mat')
load('C:\Users\Jimmy\Box Sync\share\phm competition\Data\raw data\Train mat files\Plant36.mat')
...
%--------------------------------------------------------------------------
reftotalcut=NCombPlant(windows:end,discr_ind);
[hyp_feat,~]=FDA(reftotalcut',response);
...
while j<generations && current_optim.fitness<1;
    tic
    fprintf('beginning generation %d \n',generation)
    ...
    %--------------------------------------------------------------------------
    for i=1:population
        genotype{i}.fitness=gp_fitnessmulti([features,hyp_feat],response);
    end
